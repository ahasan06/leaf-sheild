...
# Strictly use tflite-runtime (tiny interpreter only)
try:
    from tflite_runtime.interpreter import Interpreter
except Exception:
...
def get_bundle(name: str):
    ...
    CACHE[name] = bundle
    print(
        f"[INFO] {name} input: {in_shape} dtype={in_dtype} quant={in_quant}  "
        f"output dtype={out_dtype} quant={out_quant}"
    )
    return bundle


def preprocess(img: Image.Image, b):
    """Resize and normalize or quantize according to model input."""
    img = img.convert("RGB") if b["C"] == 3 else img.convert("L")
    img = img.resize((b["W"], b["H"]), Image.BILINEAR)
    x = np.array(img).astype(np.float32)
    if b["C"] == 1:
    ...
    if b["in_dtype"] == np.float32:
        x = (x / 255.0) if b["input_range"] == "0_1" else x
        return np.expand_dims(x.astype(np.float32), 0)
